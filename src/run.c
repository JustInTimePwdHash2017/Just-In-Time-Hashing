...
static void usage(const char *cmd) {
	...
	printf("\t-j\t\tUse Argon2-JIT instead of Argon2i\n");
	printf("\t-jc\t\t[JIT only] Cost in blocks per character\n");
	printf("\t-jb\t\t[JIT only] Maximum allowed backspaces\n");
...
static void run(uint32_t outlen, char *pwd, char *salt, uint32_t t_cost,
				...
				uint8_t jit, const uint32_t char_cost, const uint32_t num_del_allowed) {
    ...
    result = argon2_hash(t_cost, m_cost, threads, pwd, pwdlen, salt, saltlen,
                         ...
                         version, jit, char_cost, num_del_allowed);
...
int main(int argc, char *argv[]) {
	...
	j = 0; 
	jb = ARGON2_JIT_DEFAULT_DEL;
	jc = ARGON2_JIT_DEFAULT_COST;
    ...
    for (i = 2; i < argc; i++) {
        ...
        if (!strcmp(a, "-h")) {
			...
			if (i < argc - 1) {
                ...
                input = strtoul(argv[i], NULL, 10);
                if (input == 0 || input == ULONG_MAX) {
                    fatal("bad numeric input for -jc");
                }
                jc = input;
                continue;
            } else {
			...
			if (i < argc - 1) {
                ...
                input = strtoul(argv[i], NULL, 10);
                if (input == 0 || input == ULONG_MAX) {
                    fatal("bad numeric input for -jb");
                }
                jb = input;
                continue;
            } else {
    ...
    run(outlen, pwd, salt, t_cost, m_cost, lanes, threads, type,
       encoded_only, raw_only, version, j, jb, jc); /* TEMP */
